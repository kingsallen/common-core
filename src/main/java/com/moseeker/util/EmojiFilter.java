...
public class EmojiFilter {
     ...
     * @return
     */
    public static String refineHexString(String content) {
        StringBuffer sb = new StringBuffer(content.length());
        int len = content.length();

        for (int i = 0; i < len - 1; i++) {
            char codePoint = content.charAt(i);
            if (codePoint == '\\' && content.charAt(i + 1) == 'x') {
                i = i + 3;
            } else {
                sb.append(codePoint);
            }
        }
        sb.append(content.charAt(len - 1));
        return sb.toString();
    }
     ...
     * @return
     */
    public static JSONObject filterEmojiToJSONObject(String source) {
